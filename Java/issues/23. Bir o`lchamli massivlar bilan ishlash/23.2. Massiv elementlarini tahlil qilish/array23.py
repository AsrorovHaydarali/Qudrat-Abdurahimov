...
Massivning index1 va index2 indexlar orasidagi sonlardan tashqari qolgan sonlarning
o`rta arifmetik qiymatii chiqaruvchi dastur tuzilsin.
...
try:
    ...
    if n > 0:
        ...
        if qolgan_sonlar:
            orta_arifmetik = sum(qolgan_sonlar) / len(qolgan_sonlar)
            natija += (
                f"\n\nBu sonlarning o`rta arifmetik qiymati {orta_arifmetik} ga teng."
            )
        else:
            natija += f"\n\nMavjud bo`lmagan sonlar uchun o`rta arifmetik qiymat ham mavjud emas."
